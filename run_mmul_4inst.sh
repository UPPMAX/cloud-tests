...
ssh -i $keyfile $username@$ip1 "mpirun --map-by node --bind-to none -np 1 -host $ip1_internal,$ip2_internal,$ip3_internal,$ip4_internal,$ip1_internal,$ip2_internal,$ip3_internal,$ip4_internal ./test_matrix_manager $N $nWorkers $nThreads $cacheInGB > stdout.txt" || exit 1
