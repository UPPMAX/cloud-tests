...
if [ "$#" -ne 3 ]; then
    echo "Please give 3 arguments: N nWorkers nThreads"
    echo where
...
ssh -i $keyfile $username@$ip1 "mpirun --bind-to none -np 1 -host $ip1_internal,$ip2_internal,$ip1_internal,$ip2_internal ./test_matrix_manager $N $nWorkers $nThreads > stdout.txt" || exit 1
